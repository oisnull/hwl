...
public class CustLog {
    ...
    //    private static String MYLOG_PATH_SDCARD_DIR = "/sdcard/zl-logs";// 日志文件在sdcard中的路径
    private static char MYLOG_TYPE = 'v';// 输入日志类型，w代表只输出告警信息等，v代表输出所有信息
    private static int SDCARD_LOG_FILE_SAVE_DAYS = 10;// sd卡中日志文件的最多保存天数
    private static String FILE_EXT_NAME = ".log";// 本类输出的日志文件名称
    ...
    private static void log(String tag, String msg, char level) {
        if (BuildConfig.DEBUG) {//日志文件总开关
            ...
            if (BuildConfig.EnableFileLog)//日志写入文件开关
                writeLogtoFile(String.valueOf(level), tag, msg);
        }
    }

    public static String getCurrentFileName() {
        String needWriteFile = logfile.format(new Date());
        return needWriteFile + FILE_EXT_NAME;
    }
...
//        File dirPath = Environment.getExternalStorageDirectory();
        ...
        File dirsFile = new File(BuildConfig.LogDir);
        if (!dirsFile.exists()) {
            dirsFile.mkdirs();
        }
        //Log.i("创建文件","创建文件");
        File file = new File(dirsFile.toString(), getCurrentFileName());//
