    ...
    private void initView() {
        ...
        locationService = new BaiduLocationV2(new IHWLLoactionListener() {
            ...
            public void onSuccess(LocationModel result) {
                ...
                if (UserPosSP.getLongitude() == result.longitude &&
                        UserPosSP.getLatitude() == result.latitude) {
                    tvStatus.setText("与本地存储的位置数据一样：" + UserPosSP.getNearDesc() + " " + UserPosSP.getGroupGuid());
                    return;
                }
